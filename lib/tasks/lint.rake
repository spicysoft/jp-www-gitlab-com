...
namespace :lint do
  ...
  namespace :events do
    file = YAML.load_file(File.expand_path('../../data/events.yml', __dir__))
    ...
    def check_empty_value(file, key, skip: false)
      puts
      puts "=> Checking every event in data/events.yml for non-empty #{key}..."

      if skip
        puts 'Skipped. Issue: https://gitlab.com/gitlab-com/www-gitlab-com/-/issues/8077'
        return
      end
    ...
    def validate_date_format(file, key, skip: false)
    ...
    task :event_type do
      ...
      EVENT_TYPES = [
        'Diversity', 'Conference', 'Meetup', 'Speaking Engagement',
        'Webcast', 'Community Event', 'Virtual Conference', 'GitLab Event',
    ...
    task :date do
      validate_date_format(file, 'date', skip: true)
    end

    desc "Ensure date_ends of every event in events.yml is single ISO 8601 format"
    task :date_ends do
      validate_date_format(file, 'date_ends', skip: true)
    end
    ...
    task :social_tags do
      check_empty_value(file, 'social_tags', skip: true)
    end
    ...
    task :region do
      failed = 0
      valid_regions = %w[
        NORAM LATAM EMEA APAC Global PubSec
      ]
