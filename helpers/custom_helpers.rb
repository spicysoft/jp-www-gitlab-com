...
module CustomHelpers
  ...
  def original_view_url(site, monorepo_site_path_fragment, relative_path)
    u = "#{site.original_url}#{relative_path.sub(/\.html\.md(\.erb)?$/, '.html')}"
    u
  end

  def original_blob_editor_url(site, monorepo_site_path_fragment, relative_path)
    "#{site.original_repo}blob/master/#{monorepo_site_path_fragment}source/#{relative_path}"
  end
